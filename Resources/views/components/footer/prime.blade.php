...
<x-themes::footer class="{{ $heightclass ?? 'my-6' }}">
    ...
    <x-themes::div class="flex flex-row justify-around text-lg items-baseline text-gray-400">

        <x-themes::div id="footer_copyright">
            <x-themes::footer.item>
                {{ $copyslot ?? ('&copy; ' . (config('acp.website_author') ?? 'Yezz.Design') . ', 2022 - until now.') }}
            </x-themes::footer.item>
        </x-themes::div>
